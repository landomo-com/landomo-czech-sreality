...
export class RedisQueue {
  ...
  async pushListingIds(ids: string[]): Promise<number> {
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
      ...
      for (const id of batch) {
        ...
        pipeline.set(`${this.queueKey.replace(":queue", "")}:last_seen:${id}`, timestamp);
      }
  ...
  async updateLastSeen(id: string): Promise<void> {
    const timestamp = Date.now();
    await this.redis.set(`${this.queueKey.replace(":queue", "")}:last_seen:${id}`, timestamp);
  }
  ...
  async getLastSeen(id: string): Promise<number | null> {
    const timestamp = await this.redis.get(`${this.queueKey.replace(":queue", "")}:last_seen:${id}`);
  ...
  async pushToMissingQueue(ids: string[]): Promise<number> {
    ...
    const missingQueueKey = `${this.queueKey.replace(":queue", "")}:missing_queue`;
    const verifiedInactiveKey = `${this.queueKey.replace(":queue", "")}:verified_inactive`;
  ...
  async storePropertySnapshot(id: string, property: any): Promise<void> {
    const snapshotKey = `${this.queueKey.replace(":queue", "")}:snapshot:${id}`;
  ...
  async getChangeStats(): Promise<{
    ...
    // Count snapshots
    const pattern = `${this.queueKey.replace(":queue", "")}:snapshot:*`;
