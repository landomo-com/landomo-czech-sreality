/**
 * Sreality Missing Property Verifier - Verify and Disable Missing Properties
 *
...
export class MissingPropertyVerifier {
  ...
  async verifyProperty(id: string): Promise<boolean> {
    const baseUrl = 'https://www.sreality.com.br/api/yellow-pages/v2/search';
    const queryParams = new URLSearchParams();
    queryParams.append('house_ids', id);
    queryParams.append('availability', 'any');
    queryParams.append('business_context', "RENT");
    ...
    try {
      ...
      if (data.hits && data.hits.hits && data.hits.hits.length > 0) {
        return true; // Property still exists
      }
...
async function main() {
  ...
  logger.info('Starting Sreality Missing Property Verifier');
  ...
  try {
    ...
    // Show queue stats
    const queue = new RedisQueue('sreality');
